...
namespace BLL
{
    public delegate void DNotifyCRC(List<CRCObject> cob, List<CRCStatus> cs, string eNotify, object oNotify);//定义一个消息代理传值给UIL
    ...
    public class BLLSRMRun : BLLBase
    {
        ...
        {
            DataSet ds = dal.GetEquipmentInfo(zone);
            if (ds != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (CRCObject co in crclist)
                    {
                        co.BllSrm.ENotifyTypeEventHandler += new DNotifyType(BllSrm_ENotifyTypeEventHandler);
                        co.BllSrm.ENotify+=new BLLSRM.Notify(BllSrm_ENotify);
            ...
            while (true)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (co != null && cs.FunctionReport == CR_NO_FUNC.ToString())
                    {
                        ...
                        co.BllSrm.SendTask(equipmentnumber, row[1].ToString(), row[2].ToString().Replace("A", "").Replace("B", ""), row[3].ToString().Replace("A", "").Replace("B", ""), row[4].ToString(), row[6].ToString());
        ...
        private void BllSrm_ENotifyTypeEventHandler(string srmno, string port, string eNotify, object oNotify)
        {
            if (ENotifyCRCEventHandler != null)
                ENotifyCRCEventHandler(crclist, CRCItems, eNotify, oNotify);
        }
