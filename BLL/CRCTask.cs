...
namespace BLL
{
    public class CRCTask : BLLBase
    {
        ...
        public void UpdateCRCStatus(string num, string str, string aisle)
        {
                    ...
                    if (co != null)
                    {
                        ...
                        if (css.FunctionReport == CR_GETC_EXECUTED)
                        {
                            ...
                            //发指令调试注释
                                ...
                                co.BllSrm.SendCode(num, CR_PUTC, 0, int.Parse(css.TaskNo), destr);                             
                            //发指令调试注释
                            #endregion
                        }
                        else if (css.FunctionReport == CR_PUTC_EXECUTED)
                        {
                            ...
                            {                                    
                                ...
                                if (co.BllSrm.SendCode(num, CR_NO_FUNC, 0, int.Parse(css.TaskNo), destr))
                                {
                                    dal.UpdateTaskStatus(css.TaskNo, "SC", "1", "1");
                                    #region Tostation.Length == 4
                                    if (css.Tostation != null && css.Tostation.Length == 4)
                                    {
                                        string itemtype;
                                        ...
                                        strsta[1] = css.TaskNo;
                                        int upbarcode1 = int.Parse(info[0]) / 65536;
                                        int upbarcode2 = int.Parse(info[0]) % 65536;
            ...
            {
                CRCStatus status = new CRCStatus();
                status.CRCNum = num;
                ...
                if (co.Port == 4000)
                {
                    ...
                    if (ds != null && css.FunctionReport == CR_NO_FUNC.ToString())
                    {
                        ...
                        for (int k = 0; k < dts.Rows.Count; k++)
                        {
                            ...
                            {
                                if (CheckStation(dts.Rows[k]["fromstation"].ToString().Replace("A", ""), dts.Rows[k]["tostation"].ToString().Replace("A", ""), dts.Rows[k]["taskno"].ToString()) == dts.Rows[k]["taskno"].ToString())
                                {
