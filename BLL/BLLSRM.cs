...
namespace BLL
{
    ...
    public class BLLSRM : BLLBase
    {
        ...
        public BLLSRM(string strsrmno, string ipaddress, int port, int row)//构造函数
        {
            ...
            OSRM.AnalyseMessageEvent += new CRLSRMControl.AnalyseMessageEventHandler(OSRM_AnalyseMessageEvent);
        }
        ...
        {
            CRCObject findcob = crclist.Find(
            ...
            //if (m_strport == "80" && CommandType == "RecvData")
            {
                ...
                crc.UpdateCRCStatus(m_strSRMNo, msg.ToString(), aisle.ToString());
                if (!string.IsNullOrEmpty(crc.GetAlarmInfo(m_strSRMNo, "", false)))
                    log.WriteLog(crc.GetAlarmInfo(m_strSRMNo, "", false));
            ...
            else if (CommandType == "Break")
            {
                ...
                crc.UpdateCRCStatus(m_strSRMNo, CommandType, aisle.ToString());
        ...
        public bool SendTask(string num,string taskno, string fromstation, string tostation, string statostation,string tasktype)
        {
            try
            {
                ...
                if (crc.UpdateCRCTaskInfo(m_strSRMNo, fromstation, tostation, statostation,tasktype))
                {
