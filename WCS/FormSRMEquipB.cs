...
namespace WCS
{
    public partial class FormSRMEquipB : Form
    {
        #region 自动关闭弹出框
        ...
        BLLSRMRunB bsb = new BLLSRMRunB();
        private delegate void PictureCallBack(List<CRCObjectB> cob, List<CRCStatusB> css);
        private delegate void RichBoxCallBack(RichTextBox rb, string msg);
        string taskmode = "自动";

        public FormSRMEquipB()
        {
            InitializeComponent();
        }
        ...
        {
            bsb.ENotifyCRCEventHandler += new DNotifyCRCB(bsb_ENotifyCRCEventHandler);
            bsb.EDNotify += new BLLSRMRunB.DNotify(bsb_EDNotify);
            bsb.Start("B");
        }

        private void bsb_ENotifyCRCEventHandler(List<CRCObjectB> cob, List<CRCStatusB> css, string eNotify, object oNotify)
        {
            try
            {
                DrawConnect(cob, css);
        ...
        private void DrawConnect(List<CRCObjectB> cob, List<CRCStatusB> css)
        {
            ...
            else
            {
                ...
                foreach (CRCObjectB cco in cob)
                {
                    CRCStatusB ccs = css.Find(
                            delegate(CRCStatusB crcs)
                            {
                ...
                foreach (CRCObjectB co in cob)
                {
                    CRCStatusB findcs = css.Find(
                    ...
                    if (findcs != null)
                    {
                        int num = int.Parse(findcs.CRCNum.Substring(3, 3)) - 9;
                        int cell = 170;
                        //卫星小车载体
